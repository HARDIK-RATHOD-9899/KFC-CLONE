...
const handleEmptyCartAndCheckout = () => {
  // Fetch all cart items
  axios
    .get("https://mock-server-app-2-4ouf.onrender.com/cart")
    .then((res) => {
      const cartItems = res.data;
      // If the cart is not empty, proceed to clear it
      if (cartItems.length > 0) {
        const deletePromises = cartItems.map((item) =>
          axios.delete(`https://mock-server-app-2-4ouf.onrender.com/cart/${item.id}`)
        );
        ...
        Promise.all(deletePromises)
          .then(() => {
            Swal.fire({
              position: "center",
              icon: "success",
              title: "Your Order has given successfully",
              showConfirmButton: false,
              timer: 1500,
            }).then(() => {
              // Navigate to home after cart is cleared
              navigate("/");
            });
          })
          .catch((err) => {
        ...
        // If the cart is already empty, show a different message
        Swal.fire({
          position: "top-end",
          icon: "info",
          ...
          // Navigate to home even if the cart is empty
          navigate("/");
        });
      }
    })
  ...
  return (
    <>
         <h2 className="reciept_heading">{cart.length} ITEMS</h2>
          ...
          {path === "/cart" ? (
      <button onClick={handleEmptyCartAndCheckout} className="redButton">
      ...
      <button onClick={handlePayment} className="redButton">
        <div style={{ display: "flex", justifyContent: "space-around" }}>
          <p>Payment</p>
          <p>₹{!input ? stotal : stotal + 5}</p>
        </div>
